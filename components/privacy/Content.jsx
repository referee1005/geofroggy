import React from 'react'
import Image from 'next/image'
import Link from 'next/link'

function Content ({ data }) {
  return (
    <div className='container-custom mt-16 flex sm:flex-row flex-col gap-16'>
      <div className='block sm:hidden'>
        <div className='text-3xl font-semibold mb-12'>Table of Contents</div>
        <div className='flex flex-col gap-4 text-lg text-black '>
          <div className='flex'>
            <div>1. </div>
            <a className='underline cursor-pointer' href='#defenation'>
              Defenations
            </a>
          </div>
          <div className='flex'>
            <div>2. </div>
            <a
              className='underline cursor-pointer'
              href='#what-personal-information-we-collect'
            >
              What personal information we collect
            </a>{' '}
          </div>{' '}
          <div className='flex'>
            <div>3. </div>
            <a
              className='underline cursor-pointer'
              href='#when-we-disclose-personal-information'
            >
              When we disclose personal information
            </a>
          </div>{' '}
          <div className='flex'>
            <div>4. </div>
            <a
              className='underline cursor-pointer'
              href='#tracking-technologies-and-cookies'
            >
              Tracking Technologies and Cookies
            </a>
          </div>
          <div className='flex'>
            <div>5. </div>
            <a
              className='underline cursor-pointer'
              href='#retention-of-your-personal-data'
            >
              Retention of Your Personal Data
            </a>
          </div>{' '}
          <div className='flex'>
            <div>6. </div>
            <a
              className='underline cursor-pointer'
              href='#transfer-of-your-personal-data'
            >
              Transfer of Your Personal Data
            </a>{' '}
          </div>{' '}
          <div className='flex'>
            <div>7. </div>
            <a
              className='underline cursor-pointer'
              href='#security-of-your-personal-data'
            >
              Security of Your Personal Data
            </a>
          </div>{' '}
          <div className='flex'>
            <div>8. </div>
            <a className='underline cursor-pointer' href='#children-policy'>
              Children's Privacy
            </a>{' '}
          </div>
          <div className='flex'>
            <div>9. </div>
            <a
              className='underline cursor-pointer'
              href='#links-to-other-websites'
            >
              Links to Other Websites
            </a>
          </div>{' '}
          <div className='flex'>
            <div>10. </div>
            <a
              className='underline cursor-pointer'
              href='#changes-to-this-privacy-policy'
            >
              Changes to this Privacy Policy
            </a>
          </div>
        </div>
      </div>
      <div className='sm:w-2/3'>
        <div className='text-4xl font-semibold mb-12'>
          Privacy Policy will help you better understand how we collect, use, an
          share your personal information.
        </div>
        <div className='flex flex-col gap-12'>
          <div>
            <div className='text-3xl font-semibold mb-4'>Privacy Policy</div>
            <div className='text-lg'>
              This Privacy Policy describes Our policies and procedures on the
              collection, use and disclosure of Your information when You use
              the Service and tells You about Your privacy rights and how the
              law protects You. We use Your Personal data to provide and improve
              the Service. By using the Service, You agree to the collection and
              use of information in accordance with this Privacy Policy. This
              Privacy Policy has been created with the help of the{' '}
              <span className='underline'>Free Privacy Policy Generator</span>.
            </div>
          </div>
          <div className='privacy1'>
            <a name='defenation'></a>
            <div className='text-3xl font-semibold mb-4'>Defenations</div>
            <div className='text-lg'>
              <div className='flex'>
                <li />
                <div>
                  <span className='font-semibold'>Account</span> means a unique
                  account created for You to access our Service or parts of our
                  Service.
                </div>
              </div>
              <div className='flex'>
                <li />
                <div>
                  <span className='font-semibold'>Affiliate</span> means an
                  entity that controls, is controlled by or is under common
                  control with a party, where "control" means ownership of 50%
                  or more of the shares, equity interest or other securities
                  entitled to vote for election of directors or other managing
                  authority.
                </div>
              </div>
              <div className='flex'>
                <li />
                <div>
                  <span className='font-semibold'>Company</span> (referred to as
                  either "the Company", "We", "Us" or "Our" in this Agreement)
                  refers to Geofroggy LLC, 2164 Shingleton Road. Kentwood, MI,
                  USA.
                </div>
              </div>
              <div className='flex'>
                <li />
                <div>
                  <span className='font-semibold'>Cookies</span> are small files
                  that are placed on Your computer, mobile device or any other
                  device by a website, containing the details of Your browsing
                  history on that website among its many uses.
                </div>
              </div>
              <div className='flex'>
                <li />
                <div>
                  <span className='font-semibold'>Country</span> refers to:
                  Maryland, United States
                </div>
              </div>
              <div className='flex'>
                <li />
                <div>
                  <span className='font-semibold'> Device</span> means any
                  device that can access the Service such as a computer, a
                  cellphone or a digital tablet.
                </div>
              </div>
              <div className='flex'>
                <li />
                <div>
                  <span className='font-semibold'>Personal Data</span> is any
                  information that relates to an identified or identifiable
                  individual.
                </div>
              </div>
              <div className='flex'>
                <li />
                <div>
                  <span className='font-semibold'>Service</span> refers to the
                  Website.
                </div>
              </div>
              <div className='flex'>
                <li />
                <div>
                  <span className='font-semibold'>Service Provider</span> means
                  any natural or legal person who processes the data on behalf
                  of the Company. It refers to third-party companies or
                  individuals employed by the Company to facilitate the Service,
                  to provide the Service on behalf of the Company, to perform
                  services related to the Service or to assist the Company in
                  analyzing how the Service is used.
                </div>
              </div>
              <div className='flex'>
                <li />
                <div>
                  <span className='font-semibold'>Usage Data</span> refers to
                  data collected automatically, either generated by the use of
                  the Service or from the Service infrastructure itself (for
                  example, the duration of a page visit).
                </div>
              </div>
              <div className='flex'>
                <li />
                <div>
                  <span className='font-semibold'>Website </span>refers to
                  Geofroggy, accessible from geofroggy.com
                </div>
              </div>
              <div className='flex'>
                <li />
                <div>
                  <span className='font-semibold'>You</span> means the
                  individual accessing or using the Service, or the company, or
                  other legal entity on behalf of which such individual is
                  accessing or using the Service, as applicable.
                </div>
              </div>
            </div>
          </div>
          <div className='privacy2'>
            <a name='what-personal-information-we-collect'></a>
            <div className='text-3xl font-semibold mb-4'>
              What personal information we collect
            </div>
            <div className='text-lg'>
              This Privacy Policy describes Our policies and procedures on the
              collection, use and disclosure of Your information when You use
              the Service and tells You about Your privacy rights and how the
              law protects You. We use Your Personal data to provide and improve
              the Service. By using the Service, You agree to the collection and
              use of information in accordance with this Privacy Policy. This
              Privacy Policy has been created with the help of the Free Privacy
              Policy Generator.
            </div>
          </div>
          <div className='privacy3'>
            <a name='when-we-disclose-personal-information'></a>
            <div className='text-3xl font-semibold mb-4'>
              When we disclose personal information
            </div>
            <div className='text-lg'>
              This Privacy Policy describes Our policies and procedures on the
              collection, use and disclosure of Your information when You use
              the Service and tells You about Your privacy rights and how the
              law protects You. We use Your Personal data to provide and improve
              the Service. By using the Service, You agree to the collection and
              use of information in accordance with this Privacy Policy. This
              Privacy Policy has been created with the help of the Free Privacy
              Policy Generator.
            </div>
          </div>
          <div className='privacy4'>
            <a name='tracking-technologies-and-cookies'></a>
            <div className='text-3xl font-semibold mb-4'>
              Tracking Technologies and Cookies
            </div>
            <div className='text-lg'>
              This Privacy Policy describes Our policies and procedures on the
              collection, use and disclosure of Your information when You use
              the Service and tells You about Your privacy rights and how the
              law protects You. We use Your Personal data to provide and improve
              the Service. By using the Service, You agree to the collection and
              use of information in accordance with this Privacy Policy. This
              Privacy Policy has been created with the help of the Free Privacy
              Policy Generator.
            </div>
          </div>
          <div className='privacy5'>
            <a name='retention-of-your-personal-data'></a>
            <div className='text-3xl font-semibold mb-4'>
              Retention of Your Personal Data
            </div>
            <div className='text-lg'>
              This Privacy Policy describes Our policies and procedures on the
              collection, use and disclosure of Your information when You use
              the Service and tells You about Your privacy rights and how the
              law protects You. We use Your Personal data to provide and improve
              the Service. By using the Service, You agree to the collection and
              use of information in accordance with this Privacy Policy. This
              Privacy Policy has been created with the help of the Free Privacy
              Policy Generator.
            </div>
          </div>
          <div className='privacy6'>
            <a name='transfer-of-your-personal-data'></a>
            <div className='text-3xl font-semibold mb-4'>
              Transfer of Your Personal Data
            </div>
            <div className='text-lg'>
              This Privacy Policy describes Our policies and procedures on the
              collection, use and disclosure of Your information when You use
              the Service and tells You about Your privacy rights and how the
              law protects You. We use Your Personal data to provide and improve
              the Service. By using the Service, You agree to the collection and
              use of information in accordance with this Privacy Policy. This
              Privacy Policy has been created with the help of the Free Privacy
              Policy Generator.
            </div>
          </div>
          <div className='privacy7'>
            <a name='security-of-your-personal-data'></a>
            <div className='text-3xl font-semibold mb-4'>
              Security of Your Personal Data
            </div>
            <div className='text-lg'>
              This Privacy Policy describes Our policies and procedures on the
              collection, use and disclosure of Your information when You use
              the Service and tells You about Your privacy rights and how the
              law protects You. We use Your Personal data to provide and improve
              the Service. By using the Service, You agree to the collection and
              use of information in accordance with this Privacy Policy. This
              Privacy Policy has been created with the help of the Free Privacy
              Policy Generator.
            </div>
          </div>
          <div className='privacy8'>
            <a name='children-policy'></a>
            <div className='text-3xl font-semibold mb-4'>
              Children's Privacy
            </div>
            <div className='text-lg'>
              This Privacy Policy describes Our policies and procedures on the
              collection, use and disclosure of Your information when You use
              the Service and tells You about Your privacy rights and how the
              law protects You. We use Your Personal data to provide and improve
              the Service. By using the Service, You agree to the collection and
              use of information in accordance with this Privacy Policy. This
              Privacy Policy has been created with the help of the Free Privacy
              Policy Generator.
            </div>
          </div>
          <div className='privacy9'>
            <a name='links-to-other-websites'></a>
            <div className='text-3xl font-semibold mb-4'>
              Links to Other Websites
            </div>
            <div className='text-lg'>
              This Privacy Policy describes Our policies and procedures on the
              collection, use and disclosure of Your information when You use
              the Service and tells You about Your privacy rights and how the
              law protects You. We use Your Personal data to provide and improve
              the Service. By using the Service, You agree to the collection and
              use of information in accordance with this Privacy Policy. This
              Privacy Policy has been created with the help of the Free Privacy
              Policy Generator.
            </div>
          </div>
          <div className='privacy10'>
            <a name='changes-to-this-privacy-policy'></a>
            <div className='text-3xl font-semibold mb-4'>
              Changes to this Privacy Policy
            </div>
            <div className='text-lg'>
              This Privacy Policy describes Our policies and procedures on the
              collection, use and disclosure of Your information when You use
              the Service and tells You about Your privacy rights and how the
              law protects You. We use Your Personal data to provide and improve
              the Service. By using the Service, You agree to the collection and
              use of information in accordance with this Privacy Policy. This
              Privacy Policy has been created with the help of the Free Privacy
              Policy Generator.
            </div>
          </div>
        </div>
      </div>
      <div className='flex-1 hidden sm:block'>
        <div className='text-3xl font-semibold mb-12'>Table of Contents</div>
        <div className='flex flex-col gap-4 text-lg text-black '>
          <div className='flex'>
            <div>1. </div>
            <a className='underline cursor-pointer' href='#defenation'>
              Defenations
            </a>
          </div>
          <div className='flex'>
            <div>2. </div>
            <a
              className='underline cursor-pointer'
              href='#what-personal-information-we-collect'
            >
              What personal information we collect
            </a>{' '}
          </div>{' '}
          <div className='flex'>
            <div>3. </div>
            <a
              className='underline cursor-pointer'
              href='#when-we-disclose-personal-information'
            >
              When we disclose personal information
            </a>
          </div>{' '}
          <div className='flex'>
            <div>4. </div>
            <a
              className='underline cursor-pointer'
              href='#tracking-technologies-and-cookies'
            >
              Tracking Technologies and Cookies
            </a>
          </div>
          <div className='flex'>
            <div>5. </div>
            <a
              className='underline cursor-pointer'
              href='#retention-of-your-personal-data'
            >
              Retention of Your Personal Data
            </a>
          </div>{' '}
          <div className='flex'>
            <div>6. </div>
            <a
              className='underline cursor-pointer'
              href='#transfer-of-your-personal-data'
            >
              Transfer of Your Personal Data
            </a>{' '}
          </div>{' '}
          <div className='flex'>
            <div>7. </div>
            <a
              className='underline cursor-pointer'
              href='#security-of-your-personal-data'
            >
              Security of Your Personal Data
            </a>
          </div>{' '}
          <div className='flex'>
            <div>8. </div>
            <a className='underline cursor-pointer' href='#children-policy'>
              Children's Privacy
            </a>{' '}
          </div>
          <div className='flex'>
            <div>9. </div>
            <a
              className='underline cursor-pointer'
              href='#links-to-other-websites'
            >
              Links to Other Websites
            </a>
          </div>{' '}
          <div className='flex'>
            <div>10. </div>
            <a
              className='underline cursor-pointer'
              href='#changes-to-this-privacy-policy'
            >
              Changes to this Privacy Policy
            </a>
          </div>
        </div>
      </div>
    </div>
  )
}

export default Content
